...
- Companion skills available for the selected kind:
  - `kind=bug`: `create-bug-pr`, `close-bug-pr`
  - `kind=feature`: `create-feature-pr`, `close-feature-pr`

Inputs:

- `kind=bug|feature`.
- Bug summary + expected vs actual behavior, or feature summary + acceptance criteria.
...
- Branch created from the confirmed base branch:
  - `kind=bug` -> `fix/<slug>`
  - `kind=feature` -> `feat/<slug>`
- A GitHub PR created through the matching create skill.
- CI checks fully green; failed checks are fixed before merge.
- PR merged and cleaned up through the matching close skill.
- `deliver-pr` is successful only after merge/close is complete; create-only is not a successful delivery outcome.
...
- CI checks fail:
  - Do not merge; fix issues on the delivery branch, push, and re-run CI wait.
...
- `$AGENT_HOME/skills/workflows/pr/deliver-pr/scripts/deliver-pr.sh`

## Workflow

1. Choose kind
   - Use `kind=bug` for bugfix delivery. Branch prefix is `fix/`; create/close skills are `create-bug-pr` and `close-bug-pr`.
   - Use `kind=feature` for feature delivery. Branch prefix is `feat/`; create/close skills are `create-feature-pr` and
     `close-feature-pr`.

2. Branch-intent preflight (mandatory)
   - Run:
     - `deliver-pr.sh --kind bug preflight --base main`
     - `deliver-pr.sh --kind feature preflight --base main`
   - Optional explicit bypass run (non-blocking mode):
     - `deliver-pr.sh --kind <bug|feature> preflight --base main --bypass-ambiguity`
   - Contract:
     - This delivery method starts from `main`, creates the kind-specific branch prefix, and merges back to `main`.
...
3. Create PR
   - Use the matching create skill to:
     - create a new kind-specific branch from confirmed base
     - implement + test
     - commit + push + open PR

4. Wait for CI and repair until green
   - Run:
     - `deliver-pr.sh --kind <bug|feature> wait-ci --pr <number>`
   - If any check fails:
...
5. Close PR
   - Run:
     - `deliver-pr.sh --kind <bug|feature> close --pr <number>`
   - This delegates to the matching close skill: if draft, auto-mark ready first, then merge PR and clean branches.
...
| Infra/tooling-only edits unrelated to request | Files are only infra/tooling paths (for example `.github/`, `scripts/`, `skills/tools/`, lockfiles) while user request is product behavior.         | high       | Inspect diffs; if request linkage is unclear, block and confirm.      |
