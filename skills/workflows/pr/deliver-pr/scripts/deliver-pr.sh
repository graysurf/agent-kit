...
usage() {
  cat <<'USAGE'
Usage:
  deliver-pr.sh --kind <bug|feature> <command> [options]
...
Commands:
  ...
  close      Delegate PR merge/cleanup to the matching close-*-pr helper.

Global options:
  --kind <bug|feature>      Delivery kind. Determines branch prefix, create skill, and close helper.
...
close options:
  ...
  --skip-checks             Pass-through to the close helper.
...
require_kind() {
  ...
  case "$kind" in
    bug|feature)
      ...
      ;;
    "")
      echo "error: --kind <bug|feature> is required" >&2
      return 2
      ;;
    *)
      echo "error: invalid --kind: $kind (expected bug|feature)" >&2
      return 2
      ;;
  esac
}
...
is_infra_tooling_path() {
  local path="${1:-}"
  case "$path" in
    .github/*|scripts/*|skills/*|setup/*|.editorconfig|.pre-commit-config.yaml|.pre-commit-config.yml|.tool-versions|Makefile|package-lock.json|pnpm-lock.yaml|yarn.lock|go.sum|Cargo.lock)
      return 0
      ;;
    *)
      return 1
...
cmd_preflight() {
  ...
  collect_worktree_state
  print_worktree_state_summary
  echo "KIND=$DELIVER_PR_KIND"
  echo "BRANCH_PREFIX=$(branch_prefix_for_kind "$DELIVER_PR_KIND")"
  echo "CREATE_SKILL=$(create_skill_for_kind "$DELIVER_PR_KIND")"
  echo "CLOSE_SKILL=$(close_skill_for_kind "$DELIVER_PR_KIND")"
...
cmd_close() {
  ...
  local script_dir pr_workflow_dir close_script close_script_name close_skill
  script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  pr_workflow_dir="$(cd "$script_dir/../.." && pwd)"
  close_skill="$(close_skill_for_kind "$DELIVER_PR_KIND")"
  close_script_name="close_${DELIVER_PR_KIND}_pr.sh"
  close_script="$pr_workflow_dir/${DELIVER_PR_KIND}/${close_skill}/scripts/${close_script_name}"

  if [[ ! -x "$close_script" ]]; then
    echo "error: ${close_skill} helper not found or not executable: $close_script" >&2
    exit 1
  fi
  ...
  if [[ -n "$resolved_pr" ]]; then
    ...
    if [[ "$pr_state" == "MERGED" ]]; then
      echo "warning: ${close_skill} helper exited ${close_rc}, but PR #${resolved_pr} is already MERGED; treating close as success" >&2
      return 0
    fi
  fi
