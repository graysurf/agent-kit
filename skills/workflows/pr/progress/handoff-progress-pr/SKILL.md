...
- Use `$AGENT_HOME/skills/workflows/pr/progress/handoff-progress-pr/scripts/handoff_progress_pr.sh` to merge and patch deterministically (best-effort remote delete; local cleanup is warning-only):
  - `bash $AGENT_HOME/skills/workflows/pr/progress/handoff-progress-pr/scripts/handoff_progress_pr.sh --pr <number>`
