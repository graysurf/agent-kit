---
name: deliver-bug-pr
description: "Define the default end-to-end bug delivery method: create PR, wait/fix CI until green, then close PR."
---
...
- Bug summary + expected/actual behavior + acceptance criteria (forwarded to `create-bug-pr`).
...
- Bug-fix branch created from the confirmed base branch.
...
- CI checks fail:
  - Do not merge; fix issues on the bug-fix branch, push, and re-run CI wait.
...
3. Wait for CI and repair until green
   ...
   - If any check fails:
     - fix on the same bug-fix branch
     - push updates
     - re-run `wait-ci`
...
| Infra/tooling-only edits unrelated to request | Files are only infra/tooling paths (for example `.github/`, `scripts/`, `skills/tools/`, lockfiles) while user request is bug/product behavior. | high | Inspect diffs; if request linkage is unclear, block and confirm. |
