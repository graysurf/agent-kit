---
name: create-bug-pr
description: Create a new bug-fix branch, open a draft implementation PR early, and continue implementation in the same PR using shared bug templates.
---
...
- Required: bug summary, expected vs actual behavior, and acceptance criteria.
- Optional: kickoff artifacts already prepared (for example repro docs/tests) to commit before opening the PR.

Outputs:

- A new branch `fix/<slug>`, one or more commits, and a GitHub draft PR created via `gh`.
- PR title/body describe the bug outcome (not a housekeeping commit subject).
...
- PR body populated from the shared template: `skills/automation/find-and-fix-bugs/references/PR_TEMPLATE.md`.
...
- Missing bug summary/expected-vs-actual/acceptance criteria (cannot produce an outcome-oriented PR).
- PR title/body follows a housekeeping commit subject (for example `Add plan file`) instead of the bug outcome.
...
2. Collect required bug context:
   - bug summary, expected vs actual, and acceptance criteria (required before PR creation).
3. If required context is missing:
   - ask for a 1-2 sentence bug outcome and expected behavior.
...
- Prefer explicit user bug description and acceptance criteria.
...
- Existing plan/repro/docs files can be committed first, then used to open the draft PR.
...
1. Run preflight (above) and stop if bug summary/acceptance criteria remain missing.
...
5. If kickoff artifacts already exist (plan/repro/docs), commit them first:
...
6. Generate PR body from the shared bug template.
...
- Title: capitalize the first word; reflect the primary bug/fix outcome; never mirror a housekeeping commit subject.
- Replace the first H1 line in `skills/automation/find-and-fix-bugs/references/PR_TEMPLATE.md` with the PR title.
- Body must include: `Summary`, `Problem`, `Reproduction`, `Issues Found`, `Fix Approach`, `Testing`, and `Risk / Notes`.
...
- Use `skills/automation/find-and-fix-bugs/references/ASSISTANT_RESPONSE_TEMPLATE.md` as the response format.
