...
if [[ -n "${PROJECT_PATH:-}" ]]; then
  ...
  project_path_source="PROJECT_PATH"
fi
if [[ -n "$project_path_arg" ]]; then
  project_path="$project_path_arg"
  project_path_source="--project-path"
fi
...
echo "project_path_source: $project_path_source"
