# GraphQL API Testing Prompt (Project Context)

This file is optional. If present, the `api-test-runner` skill will read it for GraphQL project-specific context.

Keep it short, factual, and non-secret (do not paste real JWTs, passwords, API keys, or customer data).
...
- Notes for `setup/graphql/endpoints.env` presets (local/dev/staging):

## Auth / JWTs

- How to obtain tokens (login mutation/query):
