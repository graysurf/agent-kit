...
def test_tools_computer_use_google_sheets_cell_edit_readme_catalog_entry_present() -> None:
    ...
    expected_row = (
        "| Computer Use     | "
        "[google-sheets-cell-edit](./skills/tools/computer-use/google-sheets-cell-edit/) |"
    )
    assert expected_row in text


def test_tools_computer_use_google_sheets_cell_edit_skill_covers_self_improvement() -> None:
    skill_md = Path(__file__).resolve().parents[1] / "SKILL.md"
    text = skill_md.read_text(encoding="utf-8")

    assert "Skill Improvement Suggestions" in text
    assert "Follow the global Computer Use improvement rule in `AGENTS.md`" in text
