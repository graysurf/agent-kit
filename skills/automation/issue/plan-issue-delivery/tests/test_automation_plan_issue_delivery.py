...
MAIN_AGENT_PROMPT_ENV = (
    "$AGENT_HOME/skills/automation/issue/plan-issue-delivery/references/prompts/main-agent-init.md"
)
SUBAGENT_PROMPT_ENV = (
    "$AGENT_HOME/skills/automation/issue/plan-issue-delivery/references/prompts/subagent-init.md"
)
...
def test_plan_issue_delivery_e2e_sprint1_fixture_artifact_labels_and_pr_markers() -> None:
    root = repo_root()
    fixture_path = root / "tests" / "fixtures" / "issue" / "plan_issue_delivery_e2e_sprint1.md"
...
def test_plan_issue_delivery_e2e_sprint2_invariants() -> None:
    root = repo_root()
    fixture_path = root / "tests" / "fixtures" / "issue" / "plan_issue_delivery_e2e_sprint2.md"
...
SPRINT_PR_TEMPLATE_REL = "skills/automation/issue/plan-issue-delivery/references/SPRINT_PR_TEMPLATE.md"
CLOSE_PLAN_FINAL_MILE_REL = "skills/automation/issue/plan-issue-delivery/references/CLOSE_PLAN_FINAL_MILE.md"
...
def test_sprint_pr_template_reference_exists_and_documents_required_schema() -> None:
    root = repo_root()
    template = root / SPRINT_PR_TEMPLATE_REL
...
def test_sprint_pr_template_named_in_subagent_init_prompt() -> None:
    ...
    assert (
        "$AGENT_HOME/skills/automation/issue/plan-issue-delivery/references/SPRINT_PR_TEMPLATE.md"
        in text
    )
...
def test_close_plan_final_mile_reference_exists_and_lists_five_artifacts() -> None:
    root = repo_root()
    ref = root / CLOSE_PLAN_FINAL_MILE_REL
    assert ref.is_file(), (
...
def test_close_plan_final_mile_linked_from_main_agent_init_prompt() -> None:
    ...
    assert (
        "$AGENT_HOME/skills/automation/issue/plan-issue-delivery/references/CLOSE_PLAN_FINAL_MILE.md"
        in text
