...
- On first container start, the entrypoint seeds `$AGENTS_HOME` from the pinned agent-kit checkout (`$CODEX_KIT_DIR`) if the volume is empty.
...
```sh
./docker/codex-env/bin/codex-workspace up git@github.com:graysurf/agent-kit.git
```
...
CODEX_KIT_DIR=/path/to/agent-kit \
