...
This plan refactors the PR progress workflow so shared templates do not live inside a single skill directory, and so reusable helper scripts become a dedicated “tooling” skill (mirroring `plan-tooling`). The goal is to reduce coupling, make reuse explicit, and keep canonical entrypoints stable via `$AGENT_HOME/...` paths.
...
**Demo/Validation**:
- Command(s):
  - `$AGENT_HOME/scripts/lint.sh --shell`
  - `$AGENT_HOME/scripts/test.sh -k workflows_pr_progress_create_progress_pr`
- Verify:
...
- Command(s):
  - `$AGENT_HOME/scripts/lint.sh --shell`
  - `$AGENT_HOME/scripts/test.sh -k workflows_pr_progress`
- Verify:
...
- **Validation**:
  - `$AGENT_HOME/scripts/lint.sh --shell`
  - `$AGENT_HOME/scripts/test.sh -k workflows_pr_progress`
  - `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh --file skills/workflows/pr/progress/progress-tooling/SKILL.md`
  - `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh --file skills/workflows/pr/progress/create-progress-pr/SKILL.md`
