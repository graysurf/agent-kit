...
- In scope:
  ...
  - Validate three sprint topology patterns:
    ...
    - Sprint 2: parallel-capable with a single root dependency then fan-out.
...
**Goal**: validate a root dependency followed by two parallel-capable dependent tasks inside the sprint.
**Demo/Validation**:
- Command(s):
  - `AGENT_HOME="${AGENT_HOME:-$PWD}"; OUT_DIR="$AGENT_HOME/out/duck-lifecycle"; mkdir -p "$OUT_DIR"`
...
- Verify:
  - Sprint 2 batch widths are exactly `[1, 2]`.
...
- **Description**: Create dependent lane B artifact that starts after Task 2.1.
- **Dependencies**:
  - Task 2.1
- **Complexity**: 2
- **Acceptance criteria**:
...
### Task 2.3: Add dependent parallel fixture C
- **Location**:
  - `tests/issues/duck-lifecycle/sprint2/dependent-parallel/converge.md`
- **Description**: Add dependent lane C artifact that can start after Task 2.1 and run in parallel with Task 2.2.
- **Dependencies**:
  - Task 2.1
- **Complexity**: 3
- **Acceptance criteria**:
  - File exists and includes `topology: dependent-parallel`.
  - File includes `depends-on: Task 2.1`.
- **Validation**:
  - `test -f tests/issues/duck-lifecycle/sprint2/dependent-parallel/converge.md`
  - `rg -n 'topology: dependent-parallel|depends-on: Task 2.1' tests/issues/duck-lifecycle/sprint2/dependent-parallel/converge.md`
