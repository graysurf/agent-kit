...
3. `project-resolve` remains a repo-local script (installed/used via `$AGENT_HOME/scripts/project-resolve`), not a Homebrew-installed binary.
...
- **Description**: Replace `commands_dir`/`CODEX_COMMANDS_PATH` resolution with a direct call to `$AGENT_HOME/scripts/project-resolve` (with a repo-relative fallback when `AGENT_HOME` is unset) and remove dependency on `commands/project-resolve`.
- **Dependencies**:
  - Task 1.1
- **Complexity**: 5
- **Acceptance criteria**:
  - Release workflow scripts no longer reference `CODEX_COMMANDS_PATH` or `commands/project-resolve`.
  - When run inside this repo with `AGENT_HOME` unset, scripts still locate and execute `scripts/project-resolve`.
- **Validation**:
...
- **Description**: Remove `CODEX_COMMANDS_PATH` and `AGENT_HOME/commands` fallback lookup for `plan-tooling`, `semantic-commit`, and `git-scope`; require them via `command -v`/`PATH` (installed by `brew install nils-cli`) and emit clear install instructions when missing.
- **Dependencies**:
  - none
- **Complexity**: 6
- **Acceptance criteria**:
...
- **Description**: Replace runnable examples that use repo-local command paths (for example `$CODEX_COMMANDS_PATH/plan-tooling` or `$AGENT_HOME/commands/plan-tooling`) with PATH-based invocations (`plan-tooling`, `api-test`, `api-rest`, `api-gql`, `semantic-commit`, etc.), and add a brief note that these come from `brew install nils-cli`.
- **Dependencies**:
  - Task 1.3
- **Complexity**: 5
- **Acceptance criteria**:
...
- **Description**: Update workflows to install Homebrew + `graysurf/tap/nils-cli` on both macOS and Ubuntu runners, ensure Homebrew is on `PATH`, and update workflow commands to invoke `api-test`/`plan-tooling` from `PATH` (not `$AGENT_HOME/commands/...`).
- **Dependencies**:
  - Task 3.1
- **Complexity**: 8
- **Acceptance criteria**:
...
- **Validation**:
  - `rg -n \"\\$AGENT_HOME/commands/|CODEX_COMMANDS_PATH\" .github/workflows | cat`
