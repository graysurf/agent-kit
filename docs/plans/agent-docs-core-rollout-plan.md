...
This plan makes `agent-docs` the primary policy orchestration layer instead of a single `task-tools` hook. The rollout covers `AGENT_HOME` global behavior and a real project-level pilot in `/Users/terry/Project/graysurf/nils-cli`, with explicit evidence for whether project `AGENTS.md` must change. It also designs and validates an `agent-doc-init` skill to bootstrap missing baseline/policy docs safely for new projects.

## Scope

- In scope:
  - Redesign `$AGENT_HOME/AGENTS.md` and `$AGENT_HOME/AGENTS.override.md` as context dispatchers.
  - Expand `$AGENT_HOME/AGENT_DOCS.toml` to cover `startup`, `task-tools`, `skill-dev`, and `project-dev`.
  - Add context-specific policy documents under `$AGENT_HOME` and wire them via `agent-docs`.
...
2. `$AGENT_HOME` points to `/Users/terry/.config/agent-kit` in this environment.
...
1. `agent-docs resolve --context <ctx>` succeeds for all four contexts at `AGENT_HOME`.
2. `agent-docs baseline --check --target all --strict` passes at `AGENT_HOME`.
...
## Sprint 2: AGENT_HOME full-context rollout
...
**Demo/Validation**:
- Command(s):
  - `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh --file skills/tools/agent-doc-init/SKILL.md`
  - `$AGENT_HOME/skills/tools/agent-doc-init/scripts/agent_doc_init.sh --dry-run --project-path /Users/terry/Project/graysurf/nils-cli`
  - `$AGENT_HOME/skills/tools/agent-doc-init/scripts/agent_doc_init.sh --project-path /Users/terry/Project/graysurf/nils-cli`
- Verify:
...
- **Validation**:
  - `$AGENT_HOME/skills/tools/agent-doc-init/scripts/agent_doc_init.sh --dry-run --project-path /Users/terry/Project/graysurf/nils-cli`
  - `$AGENT_HOME/scripts/test.sh -k agent_doc_init`
...
1. Revert AGENTS dispatcher changes to the last known-good revision in `AGENT_HOME`.
