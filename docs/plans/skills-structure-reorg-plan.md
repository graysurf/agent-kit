...
- Ensure plan tooling entrypoints live under `commands/plan-tooling`.
  ...
  - Remove legacy entrypoints and update all runnable instructions to canonical `$AGENT_HOME/...` paths.
  - Add per-skill `tests/` for **all tracked skills** and enforce via audit + CI.
  - Update docs/tests to prefer `$AGENT_HOME/...` absolute paths for executable entrypoints.
- Out of scope:
...
4. Documentation policy: in `SKILL.md`, executable paths use `$AGENT_HOME/...` (repo-relative links are allowed for non-executables).
...
- **Description**: Define the v2 directory rules for skills, shared code, and per-skill tests.
  ...
  - Define path rules for `SKILL.md` (executables use `$AGENT_HOME/...`).
...
- **Acceptance criteria**:
  ...
  - Index only lists canonical `$AGENT_HOME/...` executable paths.
- **Validation**:
...
- **Description**: Move skill-governance scripts into a dedicated skill folder with `$AGENT_HOME/...` canonical paths.
- **Dependencies**:
  - Task 1.1
  - Task 1.2
- **Acceptance criteria**:
...
- **Description**: Add a repo check that enforces `$AGENT_HOME/...` absolute paths for executable entrypoints referenced in `SKILL.md`.
- **Dependencies**: Task 2.1
- **Acceptance criteria**:
...
- **Dependencies**:
  ...
  - Task 2.2
- **Acceptance criteria**:
  - Docs reference `$AGENT_HOME/skills/tools/skill-management/skill-governance/...` as canonical.
- **Validation**:
...
- **Description**: Do a repo-wide scan for runnable instructions and rewrite them to canonical `$CODEX_COMMANDS_PATH/...` (binaries) or `$AGENT_HOME/...` (repo scripts) paths.
- **Dependencies**:
  - Task 2.3
  - Task 2.4
- **Acceptance criteria**:
...
- **Dependencies**:
  ...
  - Task 2.1
- **Acceptance criteria**:
  - Canonical scripts run from any CWD when `AGENT_HOME` is set.
- **Validation**:
  - `scripts/test.sh -m script_smoke -k plan_to_json`

### Task 3.2: Update planning skills to use canonical `$AGENT_HOME/...` tooling
...
- Verify:
  - `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/audit-skill-layout.sh` enforces tests and passes.
...
- **Dependencies**:
  ...
  - Task 5.5
- **Acceptance criteria**:
  - `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/audit-skill-layout.sh` reports no tracked skills missing tests.
