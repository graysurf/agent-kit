...
1. Strict mode: use `agent-docs resolve --context <ctx> --strict --format checklist`. Any missing required doc is a failure signal for that
   context.
...
5. Degraded-mode execution is allowed only after an explicit strict failure on a soft-gate context, and the response must list assumptions
