...
- 2026-01-17: Clarify template placement: Markdown files with `TEMPLATE` in the filename belong in `references/` (writing skeletons) or `assets/templates/` (file scaffolds), enforced by `$AGENTS_HOME/skills/tools/skill-management/skill-governance/scripts/audit-skill-layout.sh`.
...
- Skill layout enforcement exists and passes in CI: `$AGENTS_HOME/skills/tools/skill-management/skill-governance/scripts/audit-skill-layout.sh`.
- Repo references are updated (skills/docs/tests/workflows), and validation passes:
  - `$AGENTS_HOME/scripts/lint.sh`
  - `$AGENTS_HOME/scripts/test.sh`
  - `$AGENTS_HOME/skills/workflows/pr/progress/progress-addendum/scripts/audit_progress_addendum.sh --check-updated`
...
- CI-enforced layout audit: `$AGENTS_HOME/skills/tools/skill-management/skill-governance/scripts/audit-skill-layout.sh`.
...
- Risk: broken docs/CI/scripts due to stale `template/` path references.
  - Mitigation: update all tracked references and add CI coverage (`$AGENTS_HOME/skills/tools/skill-management/skill-governance/scripts/audit-skill-layout.sh` + existing smoke tests).
...
- [x] Step 1: Implementation (skill layout + path migration)
  ...
  - Exit Criteria:
    ...
    - [x] Skill layout audit passes: `$AGENTS_HOME/skills/tools/skill-management/skill-governance/scripts/audit-skill-layout.sh`.
...
- [x] Step 3: Validation / testing
  ...
  - Artifacts:
    - `out/tests/script-coverage/summary.md` (generated by `$AGENTS_HOME/scripts/test.sh`)
  - Exit Criteria:
...
- `$AGENTS_HOME/skills/tools/skill-management/skill-governance/scripts/audit-skill-layout.sh`: Enforces tracked skill directory layout (`SKILL.md` + optional `scripts/`, `references/`, `assets/`).
- `.github/workflows/lint.yml`: Runs the skill layout audit in CI.
- `$AGENTS_HOME/skills/automation/release-workflow/scripts/release-resolve.sh`: Resolves default guide/template from the normalized locations.
