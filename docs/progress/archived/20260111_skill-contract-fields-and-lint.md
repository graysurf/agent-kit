...
- `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh` exits `0` when compliant and non-zero when any file is missing required headings.
- CI runs `shellcheck` and `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh`; lint enforces exact heading strings + fixed ordering within `## Contract`, and a minimal smoke test set is defined (no `--fix`/`--dry-run` initially).

## Scope

- In-scope:
  - Docs-only edits to `skills/**/SKILL.md` to add the minimal `## Contract` section (no behavioral changes required).
  - Add `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh` (simple, deterministic, CI-friendly).
...
- CI logs from `shellcheck` and `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh`
- Optional local report file: None (not implemented in MVP)

### Lint script contract (`$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh`)
...
- Minimal smoke tests (beyond `shellcheck`):
  - Positive: `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh` passes against the repo.
...
- [x] Step 1: Add contracts + lint script (MVP)
  - Work Items:
    ...
    - [x] Implement `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh` (list failures deterministically; exit non-zero on any failure).
  ...
  - Artifacts:
    ...
    - `scripts/README.md`
  - Exit Criteria:
    - [x] `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh` passes locally: `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh`
...
- [x] Step 2: CI integration and policy docs
  - Work Items:
    - [x] Add GitHub Actions CI job(s) to run `shellcheck` and `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh`.
...
- [x] Step 3: Validation and smoke tests
  - Work Items:
    - [x] Add minimal smoke tests without secrets:
      ...
      - Lint positive pass and negative fixture fail for `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh`
...
- `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh`: Lint enforcement for required contract headings.
