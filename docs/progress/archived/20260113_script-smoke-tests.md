...
- [x] Step 0: Alignment and inventory
  ...
  - Exit Criteria:
    ...
    - [x] Verification commands are defined: `$AGENTS_HOME/scripts/test.sh -m script_regression` and `$AGENTS_HOME/scripts/test.sh -m script_smoke`.
- [x] Step 1: Minimum viable smoke suite
  ...
  - Exit Criteria:
    - [x] At least one happy path runs end-to-end: `$AGENTS_HOME/scripts/test.sh -m script_smoke` (pass).
...
- [x] Step 2: Expand smoke coverage across scripts
  ...
  - Evidence:
    ...
    - Local `$AGENTS_HOME/scripts/test.sh -m script_smoke` runs recorded in PR Testing sections.
- [x] Step 3: CI validation and evidence
  ...
  - Exit Criteria:
    - [x] Validation commands executed with results recorded: `$AGENTS_HOME/scripts/test.sh` (pass) + CI run (pass).
...
| --- | --- | --- | --- | --- |
| `$AGENTS_HOME/scripts/build/bundle-wrapper.zsh` | `zsh -f` | regression (`--help`) | `pytest-fixture` | likely needs temp FS + wrapper inputs |
| `$AGENTS_HOME/scripts/chrome-devtools-mcp.sh` | `bash` | regression (spec: dry-run) | `spec-smoke` | keep `CHROME_DEVTOOLS_DRY_RUN=true` |
...
| `$AGENTS_HOME/skills/tools/sql/sql-mssql/scripts/sql-mssql.sh` | `bash` | regression (`--help`) | `spec-smoke` | stub `sqlcmd` to validate argv/env wiring |
| `$AGENTS_HOME/skills/tools/sql/sql-mysql/scripts/sql-mysql.sh` | `bash` | regression (`--help`) | `spec-smoke` | stub `mysql` to validate argv/env wiring |
| `$AGENTS_HOME/skills/tools/sql/sql-postgres/scripts/sql-postgres.sh` | `bash` | regression (`--help`) | `spec-smoke` | stub `psql` to validate argv/env wiring |
| `$AGENTS_HOME/scripts/env.zsh` | `zsh -f` | regression (`--help`) | `help-only` | environment helper; keep lightweight |
| `$AGENTS_HOME/scripts/test.sh` | `bash` | regression (`--help`) | `help-only` | runs deps/tests; avoid running in CI smoke |
| `$AGENTS_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh` | `bash` | functional pytest | `pytest-fixture` | already covered by `tests/test_audit_scripts.py` |
| `$AGENTS_HOME/skills/_projects/finance-report/scripts/fr-psql.zsh` | `zsh -f` | regression (`--help`) | `spec-smoke` | stub `psql` and validate flags |
| `$AGENTS_HOME/skills/_projects/megabank/scripts/mb-mssql.zsh` | `zsh -f` | regression (`--help`) | `spec-smoke` | stub `sqlcmd` and validate flags |
| `$AGENTS_HOME/skills/_projects/qburger/scripts/qb-mysql.zsh` | `zsh -f` | regression (`--help`) | `spec-smoke` | stub `mysql` and validate flags |
| `$AGENTS_HOME/skills/_projects/tun-group/scripts/tun-mssql.zsh` | `zsh -f` | regression (`--help`) | `spec-smoke` | stub `sqlcmd` and validate flags |
| `$AGENTS_HOME/skills/_projects/tun-group/scripts/tun-psql.zsh` | `zsh -f` | regression (`--help`) | `spec-smoke` | stub `psql` and validate flags |
| `$AGENTS_HOME/skills/tools/devex/desktop-notify/scripts/desktop-notify.sh` | `bash` | regression (`--help`) | `spec-smoke` | stub notifier; validate no-op behavior |
| `$AGENTS_HOME/skills/tools/devex/desktop-notify/scripts/project-notify.sh` | `bash` | regression (`--help`) | `spec-smoke` | stub notifier; validate wrapper output |
| `$AGENTS_HOME/skills/tools/devex/open-changed-files-review/scripts/open-changed-files.zsh` | `zsh -f` | regression (`--help`) | `spec-smoke` | stub `code`; validate silent no-op |
| `$AGENTS_HOME/skills/tools/devex/semantic-commit/scripts/commit_with_message.sh` | `zsh -f` | regression (`--help`) | `pytest-fixture` | needs temp git repo (creates commits) |
| `$AGENTS_HOME/skills/tools/devex/semantic-commit/scripts/staged_context.sh` | `zsh -f` | regression (`--help`) | `pytest-fixture` | needs temp git repo to stage diffs |
| `$AGENTS_HOME/skills/tools/testing/api-test-runner/scripts/api-test-summary.sh` | `bash` | regression (`--help`) | `spec-smoke` | stub inputs; validate output format |
| `$AGENTS_HOME/skills/tools/testing/api-test-runner/scripts/api-test.sh` | `bash` | regression (`--help`) | `pytest-fixture` | needs fixture manifests + stubbed runners |
| `$AGENTS_HOME/skills/tools/testing/graphql-api-testing/scripts/gql-history.sh` | `bash` | regression (`--help`) | `spec-smoke` | fixture history dir under `out/tests` |
| `$AGENTS_HOME/skills/tools/testing/graphql-api-testing/scripts/gql-report.sh` | `bash` | regression (`--help`) | `pytest-fixture` | needs fixture operations + stubbed HTTP |
| `$AGENTS_HOME/skills/tools/testing/graphql-api-testing/scripts/gql-schema.sh` | `bash` | regression (`--help`) | `pytest-fixture` | needs fixture schema + stubbed HTTP |
| `$AGENTS_HOME/skills/tools/testing/graphql-api-testing/scripts/gql.sh` | `bash` | regression (`--help`) | `pytest-fixture` | needs stubbed HTTP + fixture ops |
| `$AGENTS_HOME/skills/tools/testing/rest-api-testing/scripts/rest-history.sh` | `bash` | regression (`--help`) | `spec-smoke` | fixture history dir under `out/tests` |
| `$AGENTS_HOME/skills/tools/testing/rest-api-testing/scripts/rest-report.sh` | `bash` | regression (`--help`) | `pytest-fixture` | needs fixture requests + stubbed HTTP |
| `$AGENTS_HOME/skills/tools/testing/rest-api-testing/scripts/rest.sh` | `bash` | regression (`--help`) | `pytest-fixture` | needs stubbed HTTP + fixture files |
| `$AGENTS_HOME/skills/automation/find-and-fix-bugs/scripts/render_issues_pr.sh` | `bash` | regression (`--help`) | `spec-smoke` | fixture issues JSON input, no network |
| `$AGENTS_HOME/skills/workflows/pr/feature/close-feature-pr/scripts/close_feature_pr.sh` | `bash` | regression (`--help`) | `pytest-fixture` | stub `gh`; validate command sequencing |
| `$AGENTS_HOME/skills/workflows/pr/feature/create-feature-pr/scripts/render_feature_pr.sh` | `bash` | regression (`--help`) | `spec-smoke` | render-only; validate output format |
| `$AGENTS_HOME/skills/workflows/pr/progress/close-progress-pr/scripts/close_progress_pr.sh` | `bash` | regression (`--help`) | `pytest-fixture` | stub `gh`; fixture progress files |
| `$AGENTS_HOME/skills/workflows/pr/progress/create-progress-pr/scripts/create_progress_file.sh` | `bash` | regression (`--help`) | `pytest-fixture` | needs temp repo to avoid writing to real docs |
| `$AGENTS_HOME/skills/workflows/pr/progress/create-progress-pr/scripts/render_progress_pr.sh` | `bash` | regression (`--help`) | `spec-smoke` | render-only; validate output format |
| `$AGENTS_HOME/skills/workflows/pr/progress/create-progress-pr/scripts/validate_progress_index.sh` | `bash` | functional pytest | `pytest-fixture` | already covered by `tests/test_audit_scripts.py` |
| `$AGENTS_HOME/skills/workflows/pr/progress/handoff-progress-pr/scripts/handoff_progress_pr.sh` | `bash` | regression (`--help`) | `pytest-fixture` | stub `gh`; fixture progress PR metadata |
| `$AGENTS_HOME/skills/workflows/pr/progress/progress-addendum/scripts/audit_progress_addendum.sh` | `bash` | regression (`--help`) | `spec-smoke` | audit `docs/progress/**` for canonical addendum placement |
| `$AGENTS_HOME/skills/workflows/pr/progress/progress-addendum/scripts/progress_addendum.sh` | `bash` | regression (`--help`) | `pytest-fixture` | run against fixture progress copies under `out/tests` |
| `$AGENTS_HOME/skills/automation/release-workflow/scripts/audit-changelog.zsh` | `zsh -f` | regression (`--help`) | `spec-smoke` | run `--check` via smoke spec |
| `$AGENTS_HOME/skills/automation/release-workflow/scripts/release-audit.sh` | `bash` | regression (`--help`) | `pytest-fixture` | temp repo for tag checks + changelog fixtures |
| `$AGENTS_HOME/skills/automation/release-workflow/scripts/release-notes-from-changelog.sh` | `bash` | regression (`--help`) | `pytest-fixture` | changelog fixtures, verify extracted notes |
| `$AGENTS_HOME/skills/automation/release-workflow/scripts/release-scaffold-entry.sh` | `bash` | regression (`--help`) | `spec-smoke` | output to `out/tests` and verify content |
...
| #25 | Smoke specs: Chrome devtools + history tools | `$AGENTS_HOME/scripts/chrome-devtools-mcp.sh`, `$AGENTS_HOME/skills/tools/testing/graphql-api-testing/scripts/gql-history.sh` | Use dry-run + history fixtures under `tests/fixtures/`. |
...
| #26 | Smoke specs: release workflow audits | `$AGENTS_HOME/skills/automation/release-workflow/scripts/audit-changelog.zsh`, `.../release-scaffold-entry.sh` | Write outputs to `out/tests/script-smoke/**` and verify artifacts. |
...
| #28 | Follow-ups: PR workflow fixtures | `$AGENTS_HOME/skills/workflows/pr/feature/close-feature-pr/scripts/close_feature_pr.sh`, `$AGENTS_HOME/skills/workflows/pr/progress/handoff-progress-pr/scripts/handoff_progress_pr.sh`, `$AGENTS_HOME/skills/workflows/pr/progress/close-progress-pr/scripts/close_progress_pr.sh` | Fixture-based smoke coverage with an opt-in stubbed `gh`. |
| #29 | Follow-ups: semantic commit fixture | `$AGENTS_HOME/skills/tools/devex/semantic-commit/scripts/commit_with_message.sh` | Fixture-based smoke coverage in a temp git repo. |
| #30 | Follow-ups: API testing tool fixtures | `$AGENTS_HOME/skills/tools/testing/rest-api-testing/scripts/rest.sh`, `.../rest-report.sh`, `$AGENTS_HOME/skills/tools/testing/graphql-api-testing/scripts/gql.sh`, `.../gql-report.sh`, `.../gql-schema.sh`, `$AGENTS_HOME/skills/tools/testing/api-test-runner/scripts/api-test.sh` | Add opt-in `curl`/`xh` stubs, fixtures, and smoke specs. |
| #31 | Follow-ups: progress file helper fixtures | `$AGENTS_HOME/skills/workflows/pr/progress/create-progress-pr/scripts/create_progress_file.sh`, `skills/workflows/pr/progress/progress-addendum/scripts/*.sh` | Fixture-based smoke coverage in a temp git repo. |
| #32 | Follow-ups: release workflow fixtures | `$AGENTS_HOME/skills/automation/release-workflow/scripts/release-notes-from-changelog.sh`, `.../release-audit.sh` | Fixture-based smoke coverage with a stubbed `gh auth status`. |
| #33 | Follow-ups: bundle wrapper fixture | `$AGENTS_HOME/scripts/build/bundle-wrapper.zsh` | Fixture-based smoke coverage for bundled sources + embedded exec tools. |
