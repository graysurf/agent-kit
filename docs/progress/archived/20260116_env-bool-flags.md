...
- Add a repo-local audit and wire it into `$AGENTS_HOME/scripts/check.sh --all` to prevent regressions.
...
- Repo validation passes after implementation:
  ...
  - `$AGENTS_HOME/scripts/audit-env-bools.zsh --check`
...
- In-scope:
  ...
  - Add `$AGENTS_HOME/scripts/audit-env-bools.zsh` (ported from `/Users/terry/.config/zsh/tools/audit-env-bools.zsh`) and integrate it
    into `$AGENTS_HOME/scripts/check.sh --all`.
- Out-of-scope:
...
- Repo-local enforcement: `$AGENTS_HOME/scripts/audit-env-bools.zsh --check`.
...
- `$AGENTS_HOME/scripts/audit-env-bools.zsh`
- `out/` artifacts produced by validation (e.g. Semgrep JSON from `$AGENTS_HOME/scripts/check.sh --all`) (optional)
...
| `CHROME_DEVTOOLS_AUTOCONNECT` | `CHROME_DEVTOOLS_AUTOCONNECT_ENABLED` | `$AGENTS_HOME/scripts/chrome-devtools-mcp.sh` | Rename; strict `true|false` only. |
| `REST_HISTORY` | `REST_HISTORY_ENABLED` | `skills/tools/testing/rest-api-testing/SKILL.md`<br>`$AGENTS_HOME/skills/tools/testing/rest-api-testing/scripts/rest.sh` | Rename; update `--no-history` to set `REST_HISTORY_ENABLED=false`. |
| `REST_HISTORY_LOG_URL` | `REST_HISTORY_LOG_URL_ENABLED` | `$AGENTS_HOME/skills/tools/testing/rest-api-testing/scripts/rest.sh` | Rename; strict `true|false` only. |
| `REST_REPORT_INCLUDE_COMMAND` | `REST_REPORT_INCLUDE_COMMAND_ENABLED` | `$AGENTS_HOME/skills/tools/testing/rest-api-testing/scripts/rest-report.sh` | Rename; strict `true|false` only. |
| `REST_REPORT_COMMAND_LOG_URL` | `REST_REPORT_COMMAND_LOG_URL_ENABLED` | `$AGENTS_HOME/skills/tools/testing/rest-api-testing/scripts/rest-report.sh` | Rename; strict `true|false` only. |
| `GQL_HISTORY` | `GQL_HISTORY_ENABLED` | `skills/tools/testing/graphql-api-testing/SKILL.md`<br>`skills/tools/testing/graphql-api-testing/references/GRAPHQL_API_TESTING_GUIDE.md`<br>`$AGENTS_HOME/skills/tools/testing/graphql-api-testing/scripts/gql.sh`<br>`skills/tools/testing/graphql-api-testing/assets/scaffold/setup/graphql/gql.local.env.example` | Rename; update `--no-history` to set `GQL_HISTORY_ENABLED=false`. |
| `GQL_HISTORY_LOG_URL` | `GQL_HISTORY_LOG_URL_ENABLED` | `skills/tools/testing/graphql-api-testing/references/GRAPHQL_API_TESTING_GUIDE.md`<br>`$AGENTS_HOME/skills/tools/testing/graphql-api-testing/scripts/gql.sh`<br>`skills/tools/testing/graphql-api-testing/assets/scaffold/setup/graphql/gql.local.env.example` | Rename; strict `true|false` only. |
| `GQL_REPORT_INCLUDE_COMMAND` | `GQL_REPORT_INCLUDE_COMMAND_ENABLED` | `skills/tools/testing/graphql-api-testing/SKILL.md`<br>`skills/tools/testing/graphql-api-testing/references/GRAPHQL_API_TESTING_GUIDE.md`<br>`$AGENTS_HOME/skills/tools/testing/graphql-api-testing/scripts/gql-report.sh`<br>`skills/tools/testing/graphql-api-testing/assets/scaffold/setup/graphql/gql.local.env.example` | Rename; strict `true|false` only. |
| `GQL_REPORT_COMMAND_LOG_URL` | `GQL_REPORT_COMMAND_LOG_URL_ENABLED` | `skills/tools/testing/graphql-api-testing/references/GRAPHQL_API_TESTING_GUIDE.md`<br>`$AGENTS_HOME/skills/tools/testing/graphql-api-testing/scripts/gql-report.sh`<br>`skills/tools/testing/graphql-api-testing/assets/scaffold/setup/graphql/gql.local.env.example` | Rename; strict `true|false` only. |
| `GQL_ALLOW_EMPTY` | `GQL_ALLOW_EMPTY_ENABLED` | `$AGENTS_HOME/skills/tools/testing/graphql-api-testing/scripts/gql-report.sh`<br>`skills/tools/testing/graphql-api-testing/assets/scaffold/setup/graphql/gql.local.env.example` | Rename; strict `true|false` only. |
| `API_TEST_ALLOW_WRITES` | `API_TEST_ALLOW_WRITES_ENABLED` | `.github/workflows/api-test-runner.yml`<br>`skills/tools/testing/api-test-runner/SKILL.md`<br>`$AGENTS_HOME/skills/tools/testing/api-test-runner/scripts/api-test-summary.sh`<br>`$AGENTS_HOME/skills/tools/testing/api-test-runner/scripts/api-test.sh` | Rename; update docs/workflow examples from `1` to `true`. |
...
- [x] Step 2: Expansion / integration
  - Work Items:
    - [x] Add `$AGENTS_HOME/scripts/audit-env-bools.zsh --check` and integrate into `$AGENTS_HOME/scripts/check.sh --all`.
    - [x] Add/adjust regression tests for the new audit script (and any updated specs/stubs).
  - Artifacts:
    - `$AGENTS_HOME/scripts/audit-env-bools.zsh`
    - Updates to `$AGENTS_HOME/scripts/check.sh`
    - Test coverage under `tests/`
  - Exit Criteria:
    - [x] `$AGENTS_HOME/scripts/audit-env-bools.zsh --check` passes.
    - [x] `$AGENTS_HOME/scripts/check.sh --all` passes with audit included.
    - [x] No remaining legacy env names or forbidden values for Inventory flags outside `docs/progress/**`.
- [x] Step 3: Validation / testing
  - Work Items:
    - [x] Run and record full repo validation (`$AGENTS_HOME/scripts/check.sh --all`).
    - [x] Run and record the env-bools audit (`$AGENTS_HOME/scripts/audit-env-bools.zsh --check`).
  - Artifacts:
...
- `$AGENTS_HOME/scripts/audit-env-bools.zsh`: enforce boolean env conventions (`*_ENABLED`, `true|false` only).
- `$AGENTS_HOME/scripts/check.sh`: integrate `$AGENTS_HOME/scripts/audit-env-bools.zsh --check` into `--all`.
- `$AGENTS_HOME/scripts/chrome-devtools-mcp.sh`: adopt renamed `CHROME_DEVTOOLS_*_ENABLED` flags and strict parsing.
- `$AGENTS_HOME/skills/tools/testing/rest-api-testing/scripts/rest.sh`: adopt renamed `REST_*_ENABLED` flags and strict parsing.
- `$AGENTS_HOME/skills/tools/testing/rest-api-testing/scripts/rest-report.sh`: adopt renamed `REST_REPORT_*_ENABLED` flags and strict parsing.
- `$AGENTS_HOME/skills/tools/testing/graphql-api-testing/scripts/gql.sh`: adopt renamed `GQL_*_ENABLED` flags and strict parsing.
- `$AGENTS_HOME/skills/tools/testing/graphql-api-testing/scripts/gql-report.sh`: adopt renamed `GQL_*_ENABLED` flags and strict parsing.
- `$AGENTS_HOME/skills/tools/testing/api-test-runner/scripts/api-test.sh`: adopt renamed `API_TEST_ALLOW_WRITES_ENABLED` and strict parsing.
