...
- `$AGENT_HOME/scripts/test.sh` (or `python -m pytest`) passes locally and in GitHub Actions.
...
- In-scope:
  ...
  - Wire `pytest` into GitHub Actions so PRs and pushes run `$AGENT_HOME/scripts/test.sh`.
  - Add lightweight validations for non-code assets:
    ...
    - `skills/**/SKILL.md` contract lint remains enforced (via `$AGENT_HOME/skills/tools/skill-management/skill-governance/scripts/validate_skill_contracts.sh` and/or direct parsing).
- Out-of-scope:
...
- Optional local runner wrapper: `$AGENT_HOME/scripts/test.sh`
...
- [x] Step 0: Align scope and contracts
  ...
  - Exit Criteria:
    ...
    - [x] A minimal verification command is defined: `.venv/bin/python -m pytest` (or `$AGENT_HOME/scripts/test.sh`).
...
- [x] Step 2: Full coverage + audit checks
  - Work Items:
    - [x] Add per-script specs for scripts that cannot be validated via `--help` alone (example: `$AGENT_HOME/scripts/chrome-devtools-mcp.sh`).
