...
- `$AGENT_HOME/skills/tools/testing/api-test-runner/scripts/api-test.sh` supports an optional suite-level `auth` block that enables runtime login using a single JSON secret env var (default: `API_TEST_AUTH_JSON`).
...
- In-scope:
  ...
  - Implement runtime login + token injection in `$AGENT_HOME/skills/tools/testing/api-test-runner/scripts/api-test.sh`.
  - Minor compatibility fixes in `$AGENT_HOME/skills/tools/testing/graphql-api-testing/scripts/gql.sh` required by downstream CI usage.
...
- Out-of-scope:
  - Changes to `$AGENT_HOME/skills/tools/testing/rest-api-testing/scripts/rest.sh`.
...
- [x] Step 1: Minimum viable output (MVP)
  ...
  - Exit Criteria:
    - [x] A suite using `auth` can run end-to-end when `API_TEST_AUTH_JSON` is provided.
      - Command: `$AGENT_HOME/skills/tools/testing/api-test-runner/scripts/api-test.sh --suite <auth-suite> --out out/api-test-runner/results.json`
...
- [x] Step 3: Validation / testing
  ...
  - Exit Criteria:
    - [x] Commands executed with results recorded (local or CI):
      - `API_TEST_AUTH_JSON='...' $AGENT_HOME/skills/tools/testing/api-test-runner/scripts/api-test.sh --suite <auth-suite> --out out/api-test-runner/auth.results.json`
...
- `$AGENT_HOME/skills/tools/testing/api-test-runner/scripts/api-test.sh`: suite auth parsing, pre-login, per-case token injection, and no-leak guarantees.
