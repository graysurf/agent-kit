# agent-kit: Docker Agent env (Linuxbrew)
...
- Docs: [docker/agent-env/README.md](../../../docker/agent-env/README.md)
...
- Support running multiple isolated environments concurrently (separate `HOME`/`AGENT_HOME` state per environment).
...
- Two environments can run concurrently and do not share mutable state (verify via distinct named volumes for `HOME` and `AGENT_HOME`).
...
- Runtime secrets/state injection:
  - `OPENAI_API_KEY` (env) and/or `AGENT_HOME` (named volume) for Codex auth/session files
...
- Docker build/runtime assets (planned paths; exact layout decided in Step 0):
  ...
  - `docker/agent-env/README.md` (TL;DR usage + knobs)
  - `docker/agent-env/bin/agent-workspace` (host-side launcher; name TBD)
  - Optional: `docker/agent-env/smoke.sh` (host-invoked verification)
...
- Repository layout: `Dockerfile` + `docker-compose.yml` at repo root; keep helper scripts/docs under `docker/agent-env/`.
- `HOME`/`AGENT_HOME`: per-environment named volumes (no sharing), with `HOME=/home/agent` and `AGENT_HOME=/home/agent/.agents`.
...
- [x] Step 0: Alignment / prerequisites
  - Work Items:
    ...
    - [x] Decide repository layout for Docker assets: repo-root `Dockerfile` + `docker-compose.yml`; keep helper scripts/docs under `docker/agent-env/`.
    ...
    - [x] Decide `AGENT_HOME` strategy:
      - `HOME=/home/agent`
      - `AGENT_HOME=/home/agent/.agents`
      - One named volume per environment for `HOME` + `AGENT_HOME` (no sharing).
  ...
  - Artifacts:
    ...
    - `docker/agent-env/README.md` (planned) describing decisions and usage
...
- [x] Step 1: Minimum viable output (MVP)
  - Work Items:
    ...
    - [x] Add `docker-compose.yml` with:
      ...
      - named volume(s) for `HOME` and `AGENT_HOME`
    ...
    - [x] Create a minimal `docker/agent-env/README.md` with TL;DR commands.
  - Artifacts:
    - `Dockerfile`
    - `docker-compose.yml`
    - `docker/agent-env/README.md`
  - Exit Criteria:
    - [x] Image builds: `docker build -f Dockerfile -t agent-env:linuxbrew .`
    - [x] Interactive shell works: `docker run --rm -it agent-env:linuxbrew zsh -l`
...
- [x] Step 2: Expansion / integration
  ...
  - Work Items:
    ...
    - [x] Add compatibility shims when required (only when proven necessary by smoke tests):
      ...
      - Decision: Document-only (no shims). See `docker/agent-env/README.md`.
    ...
    - [x] Add a host-side “workspace launcher” (Option A) to quickly start an isolated workspace container from a repo input.
      - CLI / UX (interface + output):
        ...
        - [x] Define flags and environment variables (document defaults):
          ...
          - `--image <image>` / `CODEX_ENV_IMAGE` (default: `graysurf/agent-env:linuxbrew`)
      ...
      - Image / container orchestration:
        ...
        - [x] Ensure workspace state isolation:
          ...
          - One named volume per workspace for `/home/agent`.
          - One named volume per workspace for `/home/agent/.agents`.
      ...
      - GitHub auth injection at runtime:
        ...
        - [x] Provide a fallback path for SSH cloning when token is unavailable (optional):
          - Document using `docker/agent-env/docker-compose.ssh.yml` (ssh-agent forwarding + known_hosts).
      ...
      - Documentation:
        - [x] Add a dedicated section to `docker/agent-env/README.md`:
  ...
  - Artifacts:
    - `docker/agent-env/README.md` updates (document knobs, fallback policy, and known deltas vs macOS)
    - `docker/agent-env/WORKSPACE_QUICKSTART.md` (Dev Containers + Remote Tunnels quick start)
    - `docker/agent-env/bin/agent-workspace` (workspace launcher)
    - `docker/agent-env/apt-fallback.txt` (or equivalent mapping file)
    - `docker/agent-env/docker-compose.local.yml` (optional local bind-mount override)
    - `docker/agent-env/docker-compose.secrets.yml` (optional secrets override)
    - `docker/agent-env/docker-compose.ssh.yml` (optional SSH override)
    - Optional: `docker/agent-env/shims/` (only if needed)
  - Exit Criteria:
    ...
    - [x] Workspace launcher is implemented and documented:
      - `docker/agent-env/bin/agent-workspace --help` works (macOS validated; Ubuntu Server pending).
      - `agent-workspace up git@github.com:graysurf/agent-kit.git` creates a new workspace container and clones repo into `/work` (no host bind mount).
        - Evidence: `out/docker/verify/20260118_101201_workspace/workspace-launcher.log`
      - `agent-workspace tunnel <name> --detach` starts a VS Code tunnel for that workspace (first-run login flow documented).
...
- [x] Step 3: Validation / testing
  - Work Items:
    ...
    - [x] Validate `agent-kit` checks inside the container (at minimum lint):
      - `$AGENT_HOME/scripts/check.sh --lint`
    ...
    - [x] Validate the workspace launcher flow end-to-end on a clean host:
      - [x] Pull-only path (no local build): `docker pull graysurf/agent-env:linuxbrew`.
        - Evidence: `out/docker/verify/20260118_120610_pull_only/pull.log`
      - [x] Create workspace from SSH-style input: `agent-workspace up git@github.com:graysurf/agent-kit.git`.
        - Evidence: `out/docker/verify/20260118_101201_workspace/workspace-launcher.log`
      - [x] Confirm repo is in-container only:
        ...
        - Evidence: `out/docker/verify/20260118_101201_workspace/workspace-launcher.log`
      - [x] Start VS Code tunnel: `agent-workspace tunnel <name> --detach`.
        - Evidence: `out/docker/verify/20260118_101244_workspace_tunnel_wait/workspace-tunnel.log`
      ...
      - [x] Validate concurrent workspaces:
        - Two different repos or two instances of same repo can run concurrently with isolated `HOME`/`AGENT_HOME` and `/work`.
...
- `docker/agent-env`: helper scripts and runtime docs.
