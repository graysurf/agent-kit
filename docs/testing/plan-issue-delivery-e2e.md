...
- Runtime paths are exported in the current shell (`PLAN_SNAPSHOT_PATH`, `TASK_PROMPT_PATH`,
  `SUBAGENT_INIT_SNAPSHOT_PATH`, and task-scoped `DISPATCH_RECORD_PATH`).
...
4. Subagents implement assigned tasks and open/update lane PRs linked from runtime-truth rows.
   In `group` mode, lane tasks can share one PR and one `pr-shared` worktree.
...
- Grouping gate: grouped lanes keep task rows aligned to shared PR/worktree runtime facts.
